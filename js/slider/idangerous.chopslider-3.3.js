...
ChopSlider3 = function(container,params) {
	...
	//Set 3D Slider
	//cs3.v.csTransform({transform:'rotateY(0deg) rotateX(-10deg)'})
...
ChopSlider3.prototype.e.twoD = {
	...
	beads : function(cs3) {
		var sliced = cs3.h.slice({index1:cs3.h.indexes().active, square:true, squareSize:50})
		cs3.l[0].innerHTML = sliced.html
		cs3.prepare({l:1, active:0, 'new':0})
		var circles = sliced.cols*sliced.rows;
		cs3.l.children().each(function(){
			var a = $(this);
			var top = cs3.height/2 -  a.position().top - 50;
			setTimeout(function(){
				a.csTransform({
					delay:cs3.h.getDelay({type:'horizontal', index: a.index() , delay : 30, grid : sliced, startIndex : cs3.direction==1 ? 0 : sliced.cols}),
					time:300,
					transform: 'translate3d(0, '+top+'px,0) rotate(0deg)',
					ease: 'ease-in'
				})
				.css({borderRadius:50, zIndex: circles - a.index()})
				.csTransitionEnd(function(){
					setTimeout(function(){
						a.css({backgroundImage:'url('+cs3.images[cs3.newSlideIndex]+')'})
						a.csTransform({
							transform: 'translate3d(0, '+0+'px,0) rotate(0deg)',
							delay:0,
							ease : 'ease-out'
						})
						.css({borderRadius:0})
...
ChopSlider3.prototype.plugins.gallery = {
	init : function(cs3) {
		...
		if (!window.Swiper)
			$.getScript(cs3.path+'assets/idangerous.swiper-1.5.5.min.js',function(){
	...
	start : function(cs3) {
		...
		cs3._plugins.gallery.swiper.swipeTo(cs3.h.indexes().active,0)
		setTimeout(function(){
			cs3._plugins.gallery.swiper.swipeTo(cs3.h.indexes().active,0)
		},0)
		...
		function checkFS(state) {
			...
			else cs3._plugins.gallery.swiper.swipeTo(cs3.h.indexes().active,0)
		}
